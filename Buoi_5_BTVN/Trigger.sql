...
AS
	...
	ELSE
		BEGIN
			...
			ROLLBACK TRAN
		END
-- 3.9 Chỉ cho phép xóa nhân viên sinh trước năm 1966
...
AS
	...
	ELSE
		BEGIN
			...
			ROLLBACK TRAN
		END
-- 3.10 Chỉ cho phép tăng lương không quá 10%
...
AS
	...
	ELSE
		BEGIN
			...
			ROLLBACK TRAN
		END
-- 3.11 Thêm hay sửa lương nhân viên thì phải lớn hơn 0
CREATE TRIGGER KT_THEM_SUA_LUONG ON NHANVIEN
FOR INSERT, UPDATE
AS
    IF (SELECT LUONG FROM INSERTED) > 0
        COMMIT TRAN
