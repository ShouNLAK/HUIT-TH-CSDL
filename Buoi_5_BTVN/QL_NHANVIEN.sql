...
ADD CONSTRAINT DEF_THANNHAN_QUANHE DEFAULT N'Khong xac dinh' FOR QUANHE

ALTER TABLE NHANVIEN
ADD CONSTRAINT DEF_NHANVIEN_LUONG DEFAULT 8000000 FOR LUONG
...
--
-- 1. Dùng Check Constraint viết ràng buộc miền giá trị
ALTER TABLE NHANVIEN
ADD CONSTRAINT CHK_LUONG_NV CHECK (LUONG > 0)

ALTER TABLE NHANVIEN
ADD CONSTRAINT CHK_PHAI_NV CHECK (PHAI IN (N'Nam', N'Nữ'))

ALTER TABLE THANNHAN
ADD CONSTRAINT CHK_PHAI_TN CHECK (PHAI IN (N'Nam', N'Nữ'))

-- 2. Dùng Unique Constraint viết ràng buộc duy nhất cho TENPH, TENDA
ALTER TABLE PHONGBAN
ADD CONSTRAINT UNI_TENPH UNIQUE(TENPH)

ALTER TABLE DEAN
ADD CONSTRAINT UNI_TENDA UNIQUE(TENDA)

-- 3. Dùng Default Constraint viết ràng buộc giá trị mặc định
ALTER TABLE THANNHAN
ADD CONSTRAINT DF_THANNHAN_QUANHE DEFAULT N'Khong xac dinh' FOR QUANHE

ALTER TABLE NHANVIEN
ADD CONSTRAINT DF_NHANVIEN_LUONG DEFAULT 8000000 FOR LUONG

-- 4. Trigger kiểm tra ngày phân công (NGAYPC) phải >= ngày bắt đầu đề án (NGAYBD)
CREATE TRIGGER KT_NGAYPC ON PHANCONG
AFTER INSERT, UPDATE
AS
    IF EXISTS (
        SELECT I.MANV, I.MADA
        FROM INSERTED I, DEAN D
        WHERE I.MADA = D.MADA AND I.NGAYPC < D.NGAYBD
    )
        COMMIT TRAN
    ELSE
        BEGIN
            PRINT N'Lỗi: Ngày phân công phải lớn hơn hoặc bằng ngày bắt đầu đề án.'
            ROLLBACK TRAN
        END

-- 5. Trigger kiểm tra tuổi nhân viên phải >= 18 khi thêm mới
CREATE TRIGGER KT_TUOI_NV ON NHANVIEN
AFTER INSERT
AS
    IF EXISTS (
        SELECT MANV FROM INSERTED I
        WHERE YEAR(GETDATE()) - YEAR(I.NGAYSINH) < 18
    )
        COMMIT TRAN
    ELSE
        BEGIN
            PRINT N'Lỗi: Nhân viên phải từ 18 tuổi trở lên.'
            ROLLBACK TRAN
        END
